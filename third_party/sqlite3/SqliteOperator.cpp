...
CSqliteOperator& CSqliteOperator::operator<<(Blob& blob)
{
	if(_stmt_ptr)
	{
		sqlite3_bind_blob(_stmt_ptr,++_cur_bind,(const void*)blob.ptr.get(),blob.length,SQLITE_TRANSIENT);
	}
	return *this;
}
...
CSqliteOperator& CSqliteOperator::operator>>(std::string& text)
{
	if(_stmt_ptr)
	{
		char* t =NULL;
#ifdef WIN32
	t = (char*)sqlite3_column_text16(_stmt_ptr,_cur_bind++);
#else
	t = (char*)sqlite3_column_text(_stmt_ptr,_cur_bind++);
#endif	
		if( t )
		{
			text.assign(t);
...
CSqliteOperator& CSqliteOperator::operator>>(Blob& blob)
{
	if(_stmt_ptr)
	{
		...
		blob.SetData((unsigned  char*)sqlite3_column_blob(_stmt_ptr,_cur_bind++),data_length);
	}
	return *this;
}
...
//	if(_stmt_ptr)
//	{
//		int data_length = sqlite3_column_unsigned  chars(_stmt_ptr,_cur_bind);
//		blob.SetBuffer((unsigned  char*)sqlite3_column_blob(_stmt_ptr,_cur_bind++),data_length);
//	}
//	return *this;
//}
