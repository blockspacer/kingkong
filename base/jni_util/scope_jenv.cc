...
ScopeJEnv::ScopeJEnv(JavaVM* jvm, bool clear_frame, jint _capacity)
    : vm_(jvm), env_(NULL), we_attach_(false), status_(0), clear_frame_(clear_frame) {
    ASSERT(jvm);
    do {
      ...
      thread_name << "mars::" << (int)gettid();

        JavaVMAttachArgs args;
        args.group = NULL;
        args.name = thread_name.str().c_str();
