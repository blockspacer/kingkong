...
void JNU_SetField(JNIEnv *_env, jobject obj, const char *_name, const char* sig,jvalue value) {
    ...
    switch (*sig) {
        case '[':
        case 'L':
             _env->SetObjectField(obj, fid,value.l);
            break;

        case 'Z':
            _env->SetBooleanField(obj, fid,value.z);
            break;

        case 'B':
            _env->SetByteField(obj, fid,value.b);
            break;

        case 'C':
             _env->SetCharField(obj, fid,value.c);
            break;

        case 'S':
            _env->SetShortField(obj, fid,value.s);
            break;

        case 'I':
            _env->SetIntField(obj, fid,value.i);
            break;

        case 'J':
            _env->SetLongField(obj, fid,value.j);
            break;

        case 'F':
           _env->SetFloatField(obj, fid,value.f);
            break;

        case 'D':
             _env->SetDoubleField(obj, fid,value.d);
            break;
...
std::string JNU_JbyteArray2Buffer(JNIEnv *_env, const jbyteArray bytes) {
    ASSERT(_env != NULL);
    ...
    ASSERT(alen > 0);
    result.append((const char*)ba, (size_t)alen);
