#include "log.h"
#include "websocket_tls_connection_impl.h"
...
boost::asio::ssl::context WebsocketTlsConnectionImpl::s_ssl_context_(boost::asio::ssl::context::tlsv12_client);

WebsocketTlsConnectionImpl::WebsocketTlsConnectionImpl(std::unique_ptr<NetConnection::NetConnectionRequest> request,
  ...
  boost::call_once([] {
    ...
    load_root_certificates(s_ssl_context_, ec);
    }, once);
  ws_ = std::make_unique<boost::beast::websocket::stream<boost::beast::ssl_stream<boost::beast::tcp_stream> >>(GetIOService(), s_ssl_context_);
}
